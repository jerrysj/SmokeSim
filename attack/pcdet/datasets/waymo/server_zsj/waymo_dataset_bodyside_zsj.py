...
class WaymoDataset(DatasetTemplate):
    ...
    def get_infos_with_fog(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(), has_label=True, sampled_interval=1, update_info_only=False):
        from . import waymo_utils_bodyside_zsj
        print('---------------The waymo sample interval is %d, total sequecnes is %d-----------------'
              % (sampled_interval, len(self.sample_sequence_list)))

        process_single_sequence = partial(
            waymo_utils_bodyside_zsj.process_single_sequence_with_fog,
            save_path=save_path, sampled_interval=sampled_interval, has_label=has_label, update_info_only=update_info_only
        )
        sample_sequence_file_list = [
            self.check_sequence_name_with_all_version(raw_data_path / sequence_file)
        ...
        # NOTE: DEBUG 使用
        # for i in range(len(sample_sequence_file_list)): 
        #     process_single_sequence(sample_sequence_file_list[i])
        # exit(0)
...
def create_waymo_infos_with_fog(dataset_cfg, class_names, data_path, save_path,
                       raw_data_tag='raw_data', processed_data_tag='waymo_processed_data',
                       workers=min(16, multiprocessing.cpu_count()), update_info_only=False):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
    ...
    dataset.set_split(train_split)
    waymo_infos_train = dataset.get_infos_with_fog(
        raw_data_path=data_path / raw_data_tag,
        save_path=save_path / processed_data_tag, num_workers=workers, has_label=True,
        sampled_interval=1, update_info_only=update_info_only
    )
    ...
    dataset.set_split(val_split)
    waymo_infos_val = dataset.get_infos_with_fog(
        raw_data_path=data_path / raw_data_tag,
        save_path=save_path / processed_data_tag, num_workers=workers, has_label=True,
        sampled_interval=1, update_info_only=update_info_only
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--func', type=str, default='create_waymo_infos_with_fog', help='')
    ...
    if args.func == 'create_waymo_infos':
    ...
    elif args.func == 'create_waymo_infos_with_fog':
        try:
            yaml_config = yaml.safe_load(open(args.cfg_file), Loader=yaml.FullLoader)
        except:
            yaml_config = yaml.safe_load(open(args.cfg_file))
        dataset_cfg = EasyDict(yaml_config)
        dataset_cfg.PROCESSED_DATA_TAG = args.processed_data_tag
        create_waymo_infos_with_fog(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            # NOTE: 直接设置 data_path = ... / 'waymo', 会导致 567 行处相对路径无法计算
            # data_path=ROOT_DIR / 'data' / 'waymo_with_fog',
            # save_path=ROOT_DIR / 'data' / 'waymo_with_fog',
            # data_path=ROOT_DIR / 'data' / 'fog_sim_AAAI_cheshen',
            # save_path=ROOT_DIR / 'data' / 'fog_sim_AAAI_cheshen',
            # data_path=ROOT_DIR / 'data' / 'fog_sim_AAAI_offset_6',
            # save_path=ROOT_DIR / 'data' / 'fog_sim_AAAI_offset_6',
            data_path=ROOT_DIR / 'data' / 'smoke_aaai26_bodyside',
            save_path=ROOT_DIR / 'data' / 'smoke_aaai26_bodyside',
            raw_data_tag='raw_data',
            processed_data_tag=args.processed_data_tag,
            update_info_only=args.update_info_only
        )
