...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def run_eval_ops(
        ...
        sess.run(
            # NOTE: 第一个参数是 操作，feed_dict 是操作需要的变量的占位符
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100, fake_gt_infos=True):
        ...
        # # NOTE: 在这里可视化 pd 和 gt 的 bbox，看看结果是否准确
        # from .viewer.viewer import Viewer
        # for frameid_want_see in range(max(gt_frameid)):
        #     gt_bbox = gt_boxes3d[gt_frameid == frameid_want_see]
        #     sel = (pd_frameid == frameid_want_see) * (pd_score > 0.5)
        #     pd_bbox = pd_boxes3d[sel]

        #     vi = Viewer()

        #     vi.add_3D_boxes(gt_bbox, color='red', show_corner_spheres=False)
        #     vi.add_3D_boxes(pd_bbox, color='blue', show_corner_spheres=False)
        #     vi.show_3D()
        ...
        with self.test_session(graph=graph) as sess:
            ...
            self.run_eval_ops(
                sess, graph, metrics,
                pd_frameid, pd_boxes3d, pd_type, pd_score, pd_overlap_nlz,
